...
# ---------------------------------------------------------------------------
def run_batch(keyword: str, site: str = "agoda", headful: bool = False, limit: int = None):
    ...
    print("\n" + "=" * 60)
    print(f"  XONG: {len(results)} record | {len(failed)} loi | tach {n_files} file")
...
def main():
    ...
    if _is_url(target):
        ...
        run_batch(target, site=site, headful=headful, limit=limit)
