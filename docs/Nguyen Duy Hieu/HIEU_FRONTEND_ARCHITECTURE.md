...
# Canonical Source Note - 2026-06-05

This is the canonical architecture document for Nguyen Duy Hieu's Frontend Demo Tool.

The root-level `docs/HIEU_FRONTEND_ARCHITECTURE.md` is only a pointer to this file.

## Evaluation Dashboard Display Layer - Current Deliverables

Current display-only deliverables:
...
- Vu Duc Kien owns evaluation engine, metric calculation and real evaluation outputs.
- Nguyen Duy Hieu owns dashboard display, layout and mock/demo visualization.
- The frontend does not calculate Recall@10, MRR@10, NDCG@10 or any official evaluation metric.

Current mock rule:

- Values in `frontend/mock_evaluation_results.json` are `MOCK / DEMO`.
- `EvaluationDashboard.jsx` reads and displays values from JSON.
- These values remain mock/demo until Kien provides `evaluation_report`, metrics JSON or Evaluation API output.
