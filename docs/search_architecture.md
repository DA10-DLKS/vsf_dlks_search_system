...
### 4.2 Quy Trình Tìm Kiếm (Retrieve)

**Query: "Tìm khách sạn 5 sao sang trọng ở Paris"**

**Bước 1: BM25 Search**

```
Query terms: [khách, sạn, 5, sao, sang, trọng, paris]
Elasticsearch BM25: 
- Doc1 (Paris luxury 5-star hotel): 2.45 ✓
- Doc2 (Budget hotel Paris): 1.23
- Doc3 (5-star resort Bangkok): 1.89
```

**Bước 2: Vector Search**

```
Query embedding: [0.245, -0.148, 0.801, ...]
Qdrant HNSW search:
- Doc1 (Luxury hotel Paris): 0.89 ✓
- Doc3 (Premium Parisian hotel): 0.81
- Doc2 (5-star Bangkok resort): 0.72
```

**Bước 3: Hybrid Combination**

```
Normalization & Fusion:
- BM25: [0.90, 0.45, 0.70]
- Vector: [0.89, 0.81, 0.72]
- Hybrid Score (0.5*BM25 + 0.5*Vector):
  Doc1: 0.895 ✓ (Cao nhất)
  Doc3: 0.755
  Doc2: 0.575
```
...
- Doc1: relevance=0.95 ✓ Perfect match
- Doc3: relevance=0.82
- Doc2: relevance=0.48
```

**Result: Top 3 khách sạn**

1. Doc1: "Luxury 5-Star Hotel Paris" (0.95)
2. Doc3: "Premium Parisian Hotel" (0.82)
3. Doc2: "5-Star Resort Bangkok" (0.48)

### 4.3 Công Thức Hybrid Retrieval

$$Score_{Hybrid} = \alpha \cdot Score_{BM25} + (1-\alpha) \cdot Score_{Vector}$$

Với $\alpha = 0.5$ (balanced), có thể điều chỉnh:

- $\alpha = 0.7$ : Ưu tiên keyword matching
- $\alpha = 0.3$ : Ưu tiên semantic matching

---

## 5. Triển Khai Chi Tiết

### 5.1 Elasticsearch (BM25)
...
qdrant_client.create_collection(
    collection_name="travel_vectors",
...
qdrant_client.upsert(
    ...
    points=[
        Point(
            id=1,
            vector=embedding_model.encode("luxury hotel paris"),
            payload={"text": "luxury hotel paris", "doc_id": 1}
        )
    ]
)
...
results = qdrant_client.search(
    collection_name="travel_vectors",
    query_vector=embedding_model.encode("5-star hotel france"),
    limit=10
)
```
...
def hybrid_search(query: str, alpha: float = 0.5):
    ...
    bm25_scores = normalize_scores([r["_score"] for r in bm25_results])
    
    # Vector search
    vector = embedding_model.encode(query)
    vector_results = qdrant_search(vector)
    ...
    for i, doc in enumerate(bm25_results):
        ...
        hybrid_scores[doc_id] = (
            alpha * bm25_scores[i] + 
            (1 - alpha) * vector_scores.get(doc_id, 0)
        )
    
    # Reranking
    reranked = reranker.rank(
        query, 
        [bm25_results[i]["_source"]["text"] for i in range(len(bm25_results))]
    )
...
| Tiêu Chí | BM25 | Vector Search | Hybrid |
|---------|------|---------------|--------|
| **Speed** | 10-50ms | 50-200ms | 100-250ms |
| **Accuracy** | 0.65-0.75 | 0.75-0.85 | 0.85-0.92 |
| **Keyword Match** | Excellent | Poor | Excellent |
| **Semantic** | None | Excellent | Excellent |
| **Memory** | Low | High (384-dim) | High |
| **Scalability** | 100M+ docs | 10M docs | 10M docs |
| **Latency** | Sub-10ms | 50ms+ | 100ms+ |
...
- Tìm kiếm product catalog (chính xác)
- Query có keyword rõ ràng
- Cần latency thấp (<10ms)
- Large-scale indexing (100M+ docs)

**Dùng Vector Search:**

- Tìm kiếm mô tả mục tiêu
- Query natural language dài
- Xử lý typo, từ đồng nghĩa
- Semantic similarity

**Dùng Hybrid:**

- FAQ, documentation search
...
Kiến trúc **Hybrid Search** kết hợp BM25 và Vector Search cung cấp:

1. **Độ Chính Xác Cao**: Kết hợp keyword matching + semantic understanding
2. **Linh Hoạt**: Điều chỉnh α để ưu tiên keyword hoặc semantic
3. **Scalable**: Elasticsearch xử lý BM25, Qdrant xử lý vectors riêng biệt
4. **Production-Ready**: Hỗ trợ reranking, deduplication, monitoring
