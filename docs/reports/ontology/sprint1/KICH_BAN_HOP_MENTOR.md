...
"Em đã phân tích **51 khách sạn thật** crawl từ Agoda, và sản xuất ra:
- **73 khái niệm Core**, song ngữ Việt–Anh, chia **8 nhóm** (loại hình, vị trí, tiện ích, bối cảnh,
  giá, nhóm khách, phong cách, khía cạnh). Trong đó **53 hard / 20 soft**.
- **543 cách gõ** trong từ điển đồng nghĩa — cái này bàn giao cho bạn làm tầng tìm kiếm.
- **22 quan hệ 'gần'** giữa khách sạn và địa danh, để trả lời 'gần VinWonders không' bằng **đồ thị**.
- **1 hợp đồng dữ liệu** (metadata schema) — đây là **1 trong 3 contract chốt** của cả team, có code
  kiểm tra tự động bằng pydantic.
- Và **3 script tự sinh**: khi corpus mở rộng — đã đi từ 27 lên 51, sắp tới 500–1000 khách sạn —
  thì **chạy lại script thay vì sửa tay**."
...
1. **Query expansion** — em viết được **21 luật** mở rộng truy vấn, nhưng để nguyên trạng thái
   ***unverified***. Vì DA10 yêu cầu mỗi luật phải **kiểm trên bộ test golden** — luật nào không
   tăng độ phủ thì bỏ. Hiện **chưa có golden set** nên em **không tự đánh dấu 'đã kiểm'** — em chọn
   trung thực thay vì khai khống.
2. **Nhóm golden query theo facet** — cũng chờ golden set từ DA09.
3. **Verify pipeline metadata trên dữ liệu thật** — mới validate được object *mẫu*, đúng-thật chỉ
   kiểm được khi Sprint 2 chạy trên cả 51 khách sạn."

> **Câu chốt:** "Cả 3 đều **chờ một input cụ thể** chứ không phải bỏ sót — và em đã ghi rõ lộ trình mở khóa: có golden set là chạy A/B từng luật rồi chốt."
...
"Bước tiếp theo em đã chuẩn bị sẵn: viết **ontology_mapper** — tự động gắn 73 khái niệm này lên
cả corpus khách sạn, sinh ra **knowledge object hoàn chỉnh** cho tầng chunking/embedding dùng.
...
|---|---|
| "Số liệu này lấy từ đâu, có thật không?" | "Tất cả từ **51 file JSON Agoda thật** trong repo; 3 script tự sinh nên **tái lập được**, không gõ tay." |
| "Sao mới 51 khách sạn?" | "Đủ để **thiết kế** ontology vì khái niệm là vốn từ chuẩn ngành. Em thiết kế sẵn để scale 500–1000 mà không làm lại." |
...
| "Phụ thuộc ai?" | "Chờ **golden set** của DA09 để kiểm expansion; nhận **clean data** từ Data Quality; bàn giao synonym cho tầng search và metadata schema cho DA09." |
