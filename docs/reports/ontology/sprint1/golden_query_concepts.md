...
| Gap | Câu | Xử lý |
|---|---|---|
| Concept location `LOC_DA_LAT` thiếu | Q1-05, Q6-02 | Đà Lạt chưa trong corpus Sprint 1 → thêm khi mở rộng corpus. |
| Concept area `LOC_SAM_SON` (thuộc Thanh Hóa) | Q1-06 | Hiện map tạm về `LOC_THANH_HOA`. |
| Concept landmark sân bay `LMK_SAN_BAY_DA_NANG` | Q7-03 | Sân bay là landmark cho quan hệ near → bổ sung vào `location.yaml`. |
| Concept amenity "thú cưng" (pet-friendly) | Q1-05 | Vào `candidate_queue.yaml` → duyệt; ĐỒNG THỜI thiếu field `pet_policy` trong clean data (báo Data Quality). |
| Facet `aspect` chưa có câu hỏi trực tiếp | (chỉ ngầm Q2-04, Q7-01) | Bổ sung query "khách sạn sạch sẽ / dịch vụ tốt ở X" ở vòng golden tiếp theo. |
